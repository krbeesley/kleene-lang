...
behavior of the \acro{gui} is designed to help you learn the scripting language.
...
InputLabel is equal to its associated OutputLabel, so the \init{fsm}
...
This same \fsm{} can be viewed and used as an \emph{identity transducer} that maps the string
...
a way to do this.  See Appendix \ref{app:optimize}.  \init{fsm}s that
contain cycles (loops) cannot generally be determinized, but if these
cyclical
\init{fsm}s also have the ``twins property,'' they can theoretically be
...
cyclical \init{fsm}s.  It is hoped that a suitable algorithm will be
...
\fsm{}, and the integer is automatically assigned from a Unicode Private Use Area (\init{pua}).
