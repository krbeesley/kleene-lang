...
\usepackage{xltxtra}
%\usepackage{framed}
% N.B. Charis does not have a mono font face!  (Is this right?)
% try {DejaVu Sans Mono} or {Courier New} or {Lucida Sans Typewriter}
% monofont used in verb and Verbatim
% DO NOT set Mapping=tex-text in verb/Verbatim environments (it 
%	implements smart quotes and turns --- into a dash, etc.)
%\setmonofont[Scale=0.9]{DejaVu Sans Mono}
% specify ItalicFont to use \emph{} inside alltt and see oblique
%	text (there is no DejaVu Sans Mono Italic font)
\setmonofont[Scale=0.9,ItalicFont=DejaVu Sans Mono Oblique]{DejaVu Sans Mono}	
%\setmonofont[Scale=MatchLowercase]{Charis SIL}	
%\defaultfontfeatures{Mapping=tex-text} 	% for all subsequent fonts
% need Renderer=ICU to get small caps
...
\usepackage{textcomp}
% provides \textlangle{} and \textrangle{}
% rather flat angle brackets
...
\newcommand{\iostr}[1]{{\emph{#1}}}
	% used in running text, e.g.
	%" if you input \iostr{buku-buku} to the FSM, you get back
	% \iostr{buku[Noun][Redup]}"
	% This is preferable to using quotation marks, which are
	% problematic with commas and periods (in American usage)
...
\hyphenation{token-izer token-izers}
...
\include{app5}		% Work-around for right-to-left rules
